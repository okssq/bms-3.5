<template>
    <div class="operation" v-loading="loading">
        ...
        <div class="dTble">
            ...
            <Table :props ="DATA_tbProps"    v-if="btngroup[0].isActive" @setPage="GET_page_table"       @delOne="EVT_tbDelOne" @setTable="GET_table"/>
            <Img   :props ="DATA_imgProps"   v-if="btngroup[1].isActive" @setPageimg   ="GET_page_img"   @imgDelOne   ="EVT_imgDelOne"/>
            <Video :props ="DATA_videoProps" v-if="btngroup[2].isActive" @setPagevideo ="GET_page_video" @videoDelOne ="EVT_videoDelOne"/>
        </div>  
    </div>
</template>
...
<script>
    ...
    export default{
        data(){
            return {
                ...
                DATA_menuProps:{
                    title:"报警记录",
                    items:[
                        {name:"上传时间",type:"datetimerange",key:"time",value:[_mm.getDate({type:"today"}),_mm.getDate({type:"todayLast"})],picker:picker},
                        {
                            name:"车牌号码",
                            type:"vehicleList",
                            key:"license",
                        ...
                        {name:"报警类型",type:"alarmVehicle",key:"alarm",value:"",txt:"请选择"},
                        { 
                            key: "companyId", 
                            name: "公司名称", 
                            type: "singleCompany", 
        ...
        computed:{
            nowSeeType:function(){          //现在看的type类型是
        ...
        methods:{
            ...
            fn_isDeBang(isDeBang) {
                if (isDeBang == 1) {
                    this.DATA_tbProps.opeations.push({name:"删除",evt:'remove'});
                    this.DATA_imgProps.options   = [{name: '删除',evt: 'remove'}];
                    this.DATA_videoProps.options = [{name: '删除',evt: 'remove'}];
                }
            },
            //重载页面
            ReLoad(){
                ...
                this.DATA_imgProps.page.sync = 1;
                this.DATA_videoProps.page.sync = 1;
            ...
            ajax_serach(set){
                ...
                _mm.request({
                    ...
                    _this.DATA_tbProps.page.total = res.totalCount ? res.totalCount : 0;
            ...
            fn_viewList(res){
                ...
                res.results.forEach((el,index)=>{
                    arr.push({
                        set:{
                            id      : el.id,
                            imgsrc  : el.imageUrl ? el.imageUrl: '',
                            videosrc: el.ossVideoUrl ? el.ossVideoUrl: '',
                        },
                        k1:el.warningTime,
                        k4:el.warningName,
            ...
            EVT_changeShowStyle(type,index){
                ...
                for(let i=0;i<_this.btngroup.length;i++){       //设置Active样式
                    if(index === i){
                        if(_this.btngroup[i].isActive === true){return}
            ...
            EVT_allDownload(){
                ...
                window.open("/sicp/alarm/down_all_warning_info.json"+'?'+$.param(data));
            },
