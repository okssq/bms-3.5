...
export default{
        ...
        return {
            ...
            DATA_tbProps:{
                title:[
                    ...
                    {name: "里程(公里)",type: "",prop: "k2",width: ""},
                    {name: "时长(h)",type: "",prop: "k3",width: ""},
                    {name: "开始时间",type: "",prop: "k4",width: ""},
                    {name: "开始地点",type: "",prop: "k5",width: ""},
                    {name: "结束时间",type: "",prop: "k6",width: ""},
                    {name: "结束地点",type: "",prop: "k7",width: ""},
                ],
                items:[],
                page:{
                    size :10,
    ...
    created(){
        if(!this.$route.query.vehicleId){
            this.$router.push({path:'/data-center/operation'}); return;
        }
        let _name;
    ...
    methods:{
        ...
        ajax_getInfo(data){
            ...
            this.loading = true;
            _mm.request({ 
                url:'/basic/track_mileage_info.json', 
                data:data
            }).then((res)=>{
        ...
        EVT_expAll(){
            ...
            window.open("/sicp/basic/down_track_mileage_info.json"+'?'+$.param(data));
        },
        //导出全部按钮禁止效果
        fn_btnDisabled() {
            let _this = this;
